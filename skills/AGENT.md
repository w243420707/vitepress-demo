...
| Skill | 用途 | 调用场景 |
|-------|------|----------|
| `vitepress-project-standards` | 项目开发规范 | 导航栏 Emoji、Markdown 文档、代码规范、Git 提交规范 |
| `vitepress-build-checklist` | 构建前整理清单 | 构建前强制执行的检查和自动修复机制 |
...
**详细规范请调用 `vitepress-project-standards` Skill**

### 核心规范概览

1. **导航栏 Emoji 智能适配** - 调用 `vitepress-emoji-adapter` Skill
2. **Markdown 文档规范** - 文件命名、文档结构、Frontmatter
3. **代码规范** - JavaScript/TypeScript 和 Markdown 规范
4. **Git 提交规范** - 提交格式、Type 类型、示例
...
**🚨 重要：每次构建前必须强制执行整理步骤，不得跳过！**

**详细整理清单和自动修复机制请调用 `vitepress-build-checklist` Skill**

**核心要点**：
- 📋 必须逐项完成 5 大检查（Emoji、侧边栏、文档、配置、代码）
- 🔧 检查失败时自动修复，无需用户确认
- 🔄 修复后重新检查，确保问题已解决
- ✅ 所有检查通过后才能执行构建
